...
- **UID storage:** store the UID exactly as the reader returns it, lowercased hex, no separators. **7-byte UIDs (NTAG21x) are required** — the cheapest 4-byte tags can collide and must not be used as bearer credentials. The bind endpoints reject a UID shorter than 7 bytes.
