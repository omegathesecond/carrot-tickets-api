...
const PLATFORM_ONLY = [
  TicketsPermission.VIEW_USERS,
  TicketsPermission.PRINT_WRISTBANDS,
  TicketsPermission.MODERATE_SOCIAL,
];

// Regression: organizers (OWNER) must never receive the platform-staff-only
// permissions — VIEW_USERS exposes the platform-wide buyer directory,
// PRINT_WRISTBANDS mints zero-amount tickets on the office printer, and
// MODERATE_SOCIAL opens the platform-wide report queue + buyer suspension.
