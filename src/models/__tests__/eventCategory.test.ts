...
describe('Event.category', () => {
  ...
  it('defaults to events', async () => {
    ...
    expect(e.category).toBe('events');
  });
  it('stores a valid category', async () => {
    const e = await Event.create({ vendorId: new mongoose.Types.ObjectId(), name: 'B', venue: 'V', eventDate: new Date(), startTime: new Date(), endTime: new Date(), category: 'sports', ticketTypes: [] });
