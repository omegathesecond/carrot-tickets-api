...
describe('actors that are never event-restricted resolve to null', () => {
  ...
  it('a reseller OWNER token, whose operatorId is really their reseller id, when the reseller is unassigned', async () => {
    const resellerId = await seedScopedReseller([]);
...
describe('an operator with no assignment is unrestricted (empty = every event)', () => {
  ...
  it('reseller operator, under an unassigned reseller', async () => {
    const resellerId = await seedScopedReseller([]);
    ...
    const req = { reseller: { scope: 'reseller', resellerId, operatorId: (op._id as any).toString() } };
    expect(await resolveOperatorEventScope(req as any)).toBeNull();
  });
});
...
describe('an assigned operator resolves to exactly their events', () => {
  ...
  it('a token naming a gate operator that no longer exists is unrestricted, not blocked', async () => {
...
describe('operatorMayActOnEvent', () => {
  ...
  it('allows an assigned gate operator onto their event and refuses every other one', async () => {
  ...
  it('allows any event for an unassigned gate operator', async () => {
  ...
  it('refuses a restricted gate operator acting with no event at all', async () => {
